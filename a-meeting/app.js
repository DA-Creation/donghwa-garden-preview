(() => {
  ...
  const rememberMarketPosition = () => {
    ...
    const snapshot = {
      ...
      search: marketSearchInput?.value || '',
  ...
  const restoreMarketPosition = () => {
    ...
    if (marketCategoryLabels.has(snapshot.category)) activeMarketCategory = snapshot.category;
    if (typeof snapshot.search === 'string') {
      if (marketSearchInput) marketSearchInput.value = snapshot.search;
      if (marketHeaderSearchInput) marketHeaderSearchInput.value = snapshot.search;
    }
  ...
  const marketCategoryLabels = new Map([
    ['all', '전체'], ['soup', '국·찌개'], ['side', '반찬'], ['soy', '두유'], ['gift', '선물세트']
  ]);
  const marketCategoryKeys = {
    'dg-01': 'soup', 'dg-02': 'soup', 'dg-03': 'soy', 'dg-04': 'soup',
    'dg-05': 'side', 'dg-06': 'soup', 'dg-07': 'gift', 'dg-08': 'soup',
    'dg-09': 'side', 'dg-10': 'soup', 'dg-11': 'soup', 'dg-12': 'soup',
    'dg-13': 'soup', 'dg-14': 'soup', 'dg-15': 'soup', 'dg-16': 'side'
  };
  // Record the original product identity before duplicate display cards receive unique keys.
  const marketFilterData = new Map(marketAllCards.map(card => [card, {
    category: marketCategoryKeys[card.dataset.marketProductKey],
    text: [...card.querySelectorAll('h3, p')].map(node => node.textContent).join(' ').replace(/\s+/g, '').toLocaleLowerCase('ko-KR')
  }]));
  let activeMarketCategory = 'all';
  const marketCategoryNav = document.createElement('nav');
  marketCategoryNav.className = 'meeting-market-filter';
  marketCategoryNav.setAttribute('aria-label', '상품 분류');
  marketCategoryLabels.forEach((label, key) => {
    const button = document.createElement('button');
    button.type = 'button';
    button.textContent = label;
    button.dataset.marketCategory = key;
    button.setAttribute('aria-pressed', String(key === activeMarketCategory));
    marketCategoryNav.append(button);
  });
  if (marketSearchBar && marketAllGrid) marketSearchBar.after(marketCategoryNav);
  const marketFilterEmpty = document.createElement('p');
  marketFilterEmpty.className = 'meeting-market-filter-empty';
  marketFilterEmpty.textContent = '조건에 맞는 상품이 없습니다.';
  marketFilterEmpty.hidden = true;
  marketAllGrid?.after(marketFilterEmpty);
  const filterMarketProducts = () => {
    const query = marketSearchInput?.value.replace(/\s+/g, '').toLocaleLowerCase('ko-KR') || '';
    let visibleCount = 0;
    marketAllCards.forEach(card => {
      const item = marketFilterData.get(card);
      const matches = (activeMarketCategory === 'all' || item.category === activeMarketCategory) && (!query || item.text.includes(query));
      card.hidden = !matches;
      if (matches) visibleCount++;
    });
    marketCategoryNav.querySelectorAll('button').forEach(button => {
      button.setAttribute('aria-pressed', String(button.dataset.marketCategory === activeMarketCategory));
    });
    if (marketProductCount) marketProductCount.textContent = `총 ${visibleCount}개`;
    marketFilterEmpty.hidden = visibleCount !== 0;
    marketMain?.classList.toggle('is-filtering-products', activeMarketCategory !== 'all' || Boolean(query));
  };
  marketCategoryNav.addEventListener('click', event => {
    const button = event.target.closest('[data-market-category]');
    if (!button || !marketCategoryNav.contains(button)) return;
