(() => {
  ...
  function update() {
    ...
    // Keep the original portrait/title entrance, then hand the full photo to cohort.
    // The removed highlighted paragraph no longer needs its four reading beats.
    runway.style.height = `calc(220svh + ${scene.offsetHeight}px)`;
    ...
    const expand = phase(progress, .06, .36);
    const titleIn = phase(progress, 0, .10);
    const titleOut = phase(progress, .78, .96);
    ...
    guide.style.setProperty('--promise-paper', '#fff');
  }
